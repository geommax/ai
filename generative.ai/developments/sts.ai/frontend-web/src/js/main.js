...
function addEventListeners() {
    ...
    // Camera / inference events
    if (cameraToggleBtn) cameraToggleBtn.addEventListener('click', toggleCamera);
    if (inferenceBtn) inferenceBtn.addEventListener('click', triggerInference);
...
// Start camera and show video in the camera panel
async function startCamera() {
    try {
        ...
        if (cameraVideo) {
            ...
            // Ensure video plays (muted allows autoplay in many browsers)
            cameraVideo.muted = true;
            try { await cameraVideo.play(); } catch (e) { /* ignore autoplay issues */ }
        }
        if (cameraStatus) {
            cameraStatus.textContent = 'Active';
            cameraStatus.classList.remove('opacity-60');
            cameraStatus.classList.add('text-success');
        }
        if (cameraToggleBtn) {
            // Make camera toggle appear active (use red like STS stop)
            cameraToggleBtn.classList.remove('bg-gray-200', 'text-gray-700');
            cameraToggleBtn.classList.add('bg-danger', 'hover:bg-red-700', 'text-white');
            cameraToggleBtn.innerHTML = '<i class="fas fa-video-slash"></i>';
            cameraToggleBtn.setAttribute('aria-label', 'Stop Camera');
            cameraToggleBtn.title = 'Stop Camera';
        }
    } catch (error) {
        console.error('Error starting camera:', error);
        showNotification('Unable to access camera. Please grant permission and ensure you are using HTTPS.', 'error');
        if (cameraStatus) {
...
function stopCamera() {
    try {
        ...
        if (cameraStatus) {
            ...
            cameraStatus.classList.add('opacity-60');
            cameraStatus.classList.remove('text-success');
        }
        if (cameraToggleBtn) {
            cameraToggleBtn.classList.remove('bg-danger', 'hover:bg-red-700', 'text-white');
            cameraToggleBtn.classList.add('bg-gray-200');
            cameraToggleBtn.innerHTML = '<i class="fas fa-video"></i>';
            cameraToggleBtn.setAttribute('aria-label', 'Start Camera');
            cameraToggleBtn.title = 'Start Camera';
        }
    } catch (error) {
...
// Inference button handler (repurposed stop-cam)
function triggerInference() {
    showNotification('Inference triggered', 'info');
    // Placeholder: wire to actual inference flow as needed
}
...
async function startRecording() {
    try {
        ...
        // Update UI (icon-only + tooltip)
        speechToggleBtn.classList.remove('bg-primary', 'hover:bg-blue-700');
        speechToggleBtn.classList.add('bg-danger', 'hover:bg-red-700', 'text-white');
        speechToggleBtn.innerHTML = '<i class="fas fa-stop"></i>';
        speechToggleBtn.title = 'Stop STS';
...
function stopRecording() {
    if (mediaRecorder && isRecording) {
        ...
        // Update UI (icon-only + tooltip)
        speechToggleBtn.classList.remove('bg-danger', 'hover:bg-red-700', 'text-white');
        speechToggleBtn.classList.add('bg-primary', 'hover:bg-blue-700');
        speechToggleBtn.innerHTML = '<i class="fas fa-microphone"></i>';
        speechToggleBtn.title = 'Start STS';
