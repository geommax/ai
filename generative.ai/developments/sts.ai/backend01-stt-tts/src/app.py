...
# Allow user to override models directory; default to ~/piper_models
PIPER_MODELS_DIR = os.getenv("PIPER_MODELS_DIR", os.path.expanduser("~/piper_models"))

# Locate a .onnx model and optional JSON config in the models directory
    ...
    if os.path.isdir(PIPER_MODELS_DIR):
        for fname in os.listdir(PIPER_MODELS_DIR):
            if fname.endswith('.onnx'):
                PIPER_MODEL_PATH = os.path.join(PIPER_MODELS_DIR, fname)
                # Try common json config names
                json_candidates = [f"{fname}.json", os.path.splitext(fname)[0] + ".onnx.json", os.path.splitext(fname)[0] + ".json"]
                for jc in json_candidates:
                    jc_path = os.path.join(PIPER_MODELS_DIR, jc)
                    if os.path.exists(jc_path):
                        PIPER_MODEL_CONFIG = jc_path
                        break
                break
except Exception as e:
    logger.warning(f"Error scanning Piper models dir {PIPER_MODELS_DIR}: {e}")

# Check if Piper binary is installed and model file exists
try:
    ...
    if PIPER_MODEL_PATH and os.path.exists(PIPER_MODEL_PATH):
        ...
        logger.info(f"Piper TTS initialized with model: {PIPER_MODEL_PATH}")
    else:
        logger.warning(f"Piper binary found but no model files in {PIPER_MODELS_DIR}")
except (subprocess.CalledProcessError, FileNotFoundError):
    logger.warning("Piper TTS binary not available")
